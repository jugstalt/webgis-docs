...
Hier werden alle verfügbaren Ausdehnung aus dem CMS aufgelistet. Durch anklicken eines Elements wird diesen ausgewählt.
...
wie bei einem vorprozessierten *Tiling* Dienst. Damit die App beim Verändern des Kartenausschnittes trotzdem flüssig wirkt, muss hier Hintergrund Dienst angegeben werden,
...
Damit Daten aus dem Dienst vergleichen werden können, muss der Dienst links und rechts mit unterschiedlichen Layerschaltungen dargestellt werden. Welche Layer das sind,
...
Der Anwender kann in der App dann für Seite die gewünschte Ansicht wählen:

.. image:: img/side-by-side2.png

Für diese Ansichten werden nicht die Darstellungsvarianten für diesen Dienst aus dem CMS herangezogen, sondern die einzelnen Schaltungen werden hier hier im *Json* Format eingetragen.
Öffnet man den *JSON Editor* zuerst einen Beispiel eingefügt werden, dass dann auf die eigenten Bedürfnisse angepasst werden kann:

.. image:: img/side-by-side3.png

.. note::
   Bei JSON handelt es sich die Javascript Beschreibung eines Objekte. Das eingefügte Beispiel ist zuerst noch *auskommentiert*. Um es zu verwenden, müssen zuerst die Zeilen mit ``/*`` und ``*/`` entfernt werden
...
Da es sich bei den Ansichten um eine Auflisting (Array) handelt, müssen die Element mit Bestrich getrennt werden und von eckigen Klammen umgeben sein. Die einzelnen Ansichten Element werden durch geschwungene
