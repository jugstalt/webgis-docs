...
* **Gelb**: Für diesen Knoten wurden Berechtigungen definiert, die allerdings keine Benutzer beschränken. Dieser Knoten ist immer noch für alle Anwender sichbar.
...
übergeordneten Knoten wie ``Dienste`` sind. Dort könnte die Berechtigung etwa folgendermaßen aussehen:

.. image:: img/security9.png

Hier wird zusätzlich zu ``Jeder`` noch der Benutzer ``subscriber::map-author`` hinzugefügt. Dieser ist beispielsweise der Karten Autor uns sollte natürlich auch alle
...
**Rote** Berechtigungs-Buttons gibt es zusätzlich noch in einer etwas *blaseren* Darstellung:
